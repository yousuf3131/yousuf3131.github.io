// Room networking for Tanks. One player hosts the room in their browser; everyone else connects to them.
//
// Two ways to reach the host:
//   1. Direct (WebRTC via PeerJS): fastest, used whenever the two networks allow it.
//   2. Relay (MQTT over a secure WebSocket): used automatically when a direct
...
const topicRoot = code => `yoahka-tanks/v1/${code}`;
...
function relayConnect(url) {
    return new Promise((resolve, reject) => {
        const client = window.mqtt.connect(url, { connectTimeout: 6000, reconnectPeriod: 2000, clean: true, clientId: `tanks_${Math.random().toString(16).slice(2, 12)}` });
...
export class HostNet {
    ...
    open(code) {
        if (!window.Peer) return Promise.reject(new Error('The multiplayer library did not load. Check your connection, or battle the bots solo.'));
        return new Promise((resolve, reject) => {
            const peer = new window.Peer(PREFIX + code, { debug: 0, config: ICE });
            this.peer = peer;
            const timer = setTimeout(() => reject(new Error('Could not reach the multiplayer server. Try again, or battle the bots solo.')), 12000);
            peer.on('open', () => {
                clearTimeout(timer);
            ...
            peer.on('error', err => {
                clearTimeout(timer);
                reject(new Error(err.type === 'unavailable-id' ? 'code-taken' : 'Could not reach the multiplayer server. Try again, or battle the bots solo.'));
            });
