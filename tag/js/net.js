// Room networking for Tag Royale. Adapted from Drawing Duel.
// See draw/js/net.js for full commentary.

const PREFIX = 'yoahka-tag-';
...
const topicRoot = code => `yoahka-tag/v1/${code}`;
...
function relayConnect(url) {
    return new Promise((resolve, reject) => {
        const client = window.mqtt.connect(url, { connectTimeout: 6000, reconnectPeriod: 2000, clean: true, clientId: `tag_${Math.random().toString(16).slice(2, 12)}` });
        client.on('error', () => {});
