// Room networking for Drawing Duel. Adapted from Bonk Racers.
// See race/js/net.js for full commentary.

const PREFIX = 'yoahka-draw-';
...
const topicRoot = code => `yoahka-draw/v1/${code}`;
...
function relayConnect(url) {
    return new Promise((resolve, reject) => {
        const client = window.mqtt.connect(url, { connectTimeout: 6000, reconnectPeriod: 2000, clean: true, clientId: `draw_${Math.random().toString(16).slice(2, 12)}` });
        client.on('error', () => {});
