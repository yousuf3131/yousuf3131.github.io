// Room networking for Archery Duel. Adapted from Drawing Duel / Bonk Racers.
// See race/js/net.js for full commentary.

const PREFIX = 'yoahka-archery-';
...
const topicRoot = code => `yoahka-archery/v1/${code}`;
...
function relayConnect(url) {
    return new Promise((resolve, reject) => {
        const client = window.mqtt.connect(url, { connectTimeout: 6000, reconnectPeriod: 2000, clean: true, clientId: `archery_${Math.random().toString(16).slice(2, 12)}` });
        client.on('error', () => {});
